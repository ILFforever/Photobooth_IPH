...
export type SequenceState = 'idle' | 'scramble' | 'countdown' | 'capturing' | 'photoConfirm' | 'waitingForPreview' | 'review' | 'preCountdown' | 'complete' | 'manualReview';
...
interface UsePhotoboothSequenceReturn {
  ...
  adjustCountdown: (delta: number) => void;  // Temporarily adjust live countdown (only during countdown state)
...
export function usePhotoboothSequence(config: SequenceConfig): UsePhotoboothSequenceReturn {
  ...
  // Temporarily adjust the live countdown value (only works during countdown state)
  // Up: adds time, Down: removes time, minimum 3 seconds
  const adjustCountdown = useCallback((delta: number) => {
    if (sequenceState !== 'countdown') return;
    setCurrentCountdown(prev => Math.max(3, prev + delta));
  }, [sequenceState]);
  ...
  // Auto review: countdown review time, then go to preCountdown for next photo
  useEffect(() => {
          ...
          if (prev <= 1) {
            ...
            setPhotosTaken(currentTaken => {
              if (currentTaken >= config.autoCount) {
                ...
                logger.debug('[PhotoboothSequence] Auto review ended, continuing to next photo');
