...
function EditableZone({ zone, zIndex, frameWidth, frameHeight, isSelected, onSelect, onUpdate, scale, snapEnabled }: EditableZoneProps) {
  ...
  const SNAP_THRESHOLD = 8; // screen pixels (scaled by zoom for canvas coordinates)
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (isDragging) {
        ...
        if (snapEnabled !== false) {
          ...
          // When zoomed in, we need a smaller threshold in canvas coordinates
          const adjustedSnapThreshold = SNAP_THRESHOLD / zoomScale;

          // Check for center snap (zone center)
          const zoneCenterX = rawX + zoneStart.width / 2;
          const zoneCenterY = rawY + zoneStart.height / 2;

          const snappedCenterX = applySnap(zoneCenterX, centerX, adjustedSnapThreshold);
          const snappedCenterY = applySnap(zoneCenterY, centerY, adjustedSnapThreshold);

          // Check for edge snaps
          const snappedLeft = applySnap(rawX, 0, adjustedSnapThreshold);
          const snappedRight = applySnap(rawX + zoneStart.width, frameWidth, adjustedSnapThreshold);
          const snappedTop = applySnap(rawY, 0, adjustedSnapThreshold);
          const snappedBottom = applySnap(rawY + zoneStart.height, frameHeight, adjustedSnapThreshold);
        ...
        switch (isResizing) {
          case 'resize-e':
            ...
            const snappedRightE = snapEnabled !== false ? applySnap(rightEdgeE, frameWidth, adjustedSnapThreshold) : rightEdgeE;
          ...
          case 'resize-s':
            ...
            const snappedBottomS = snapEnabled !== false ? applySnap(bottomEdgeS, frameHeight, adjustedSnapThreshold) : bottomEdgeS;
          ...
          case 'resize-w':
            ...
            const snappedLeftW = snapEnabled !== false ? applySnap(leftEdgeW, 0, adjustedSnapThreshold) : leftEdgeW;
          ...
          case 'resize-n':
            ...
            const snappedTopN = snapEnabled !== false ? applySnap(topEdgeN, 0, adjustedSnapThreshold) : topEdgeN;
  ...
  return (
    ...
    >
      ...
      {/* Guides are positioned relative to the zone, so we offset to canvas coordinates */}
