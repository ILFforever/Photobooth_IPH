...
import { useCollage, CANVAS_SIZES, CanvasSize, FloatingPanelType } from "../../../contexts";
import { Frame } from "../../../types/frame";
import { Background } from "../../../types/background";
import { CustomCanvasDialog } from "../dialogs";
...
const FloatingFrameSelector = () => {
  ...
  // Sync current frame with frames list (for imported custom sets)
  // When a custom set is loaded, the frame might have a different ID
  useEffect(() => {
    if (!currentFrame || frames.length === 0) return;

    // Check if current frame ID exists in the frames list
    const idMatch = frames.find(f => f.id === currentFrame.id);
    if (idMatch) return; // Already matched by ID

    // Try to match by properties (zones structure, dimensions, name)
    const matchByProperties = frames.find(f => {
      if (f.name !== currentFrame.name) return false;
      if (f.width !== currentFrame.width || f.height !== currentFrame.height) return false;
      if (f.zones.length !== currentFrame.zones.length) return false;
      // Check zone structure
      return JSON.stringify(f.zones) === JSON.stringify(currentFrame.zones);
    });

    if (matchByProperties) {
      logger.debug('[FloatingFrameSelector] Syncing frame from imported set:', matchByProperties.name);
      setCurrentFrame(matchByProperties);
    } else {
      // Frame not found in list (e.g. imported from another machine) — add it
      logger.debug('[FloatingFrameSelector] Adding imported frame to list:', currentFrame.name);
      setFrames(prev => [...prev, currentFrame]);
    }
  }, [currentFrame, frames]);
  ...
  const getCurrentBackground = () => {
    ...
    // Normalize path for comparison - handle all Tauri asset URL formats
    ...
    // First try to find in the backgrounds array
    const found = backgrounds.find((bg) => {
      return normalizePath(bg.value) === normalizePath(background);
    });

    if (found) return found;

    // Fallback: create a temporary Background object with the name extracted from the path
    // This handles custom set backgrounds that aren't in the main backgrounds array
    const extractNameFromPath = (path: string): string => {
      try {
        const cleanPath = decodeURIComponent(path)
          .replace(/^(asset:\/\/|http:\/\/asset\.localhost\/)/, '')
          .replace(/\\/g, '/');
    ...
    return {
      id: 'temp',
      name: extractNameFromPath(background),
      description: '',
  ...
  const handleDropImageOnBackground = async (item: { path: string; thumbnail: string; dimensions?: { width: number; height: number } }) => {
    try {
      ...
      invoke<Background>("import_background", {
        ...
        // Update the background value to match the imported background
        // This ensures getCurrentBackground() can find it with the correct name
        const convertedNewBgSrc = convertFileSrc(newBg.value.replace('asset://', ''));
        setBackground(convertedNewBgSrc);
                          ...
                          >
                            <div className="frame-option-content">
                              <div className="frame-preview-container" style={{ position: 'relative' }}>
                                ...
                                >
                                  {bg.background_type === "image" &&
                                    (bg.thumbnail || bg.value.startsWith('asset://')) && (
                                      <img
